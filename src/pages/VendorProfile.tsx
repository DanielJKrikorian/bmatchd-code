...
import { 
  Heart, MapPin, DollarSign, Star, MessageSquare, Globe, 
  Facebook, Instagram, Youtube, Loader2, Send, ArrowLeft, ArrowRight 
} from 'lucide-react';
...
const VendorProfile = () => {
  ...
  const loadVendorData = async () => {
    ...
    try {
      ...
      if (vendorError) {
        console.error('[LOAD] Vendor fetch error:', vendorError);
      ...
      setReviews((vendorData.reviews || []).map(review => ({
        ...review,
        couple: review.couple && review.couple.length > 0 
          ? review.couple as { partner1_name: string; partner2_name: string }[] 
          : null,
      })));
      ...
      if (packagesError) {
        console.error('[LOAD] Packages fetch error:', packagesError);
      ...
      if (authUser.user) {
        ...
        const { data: userData, error: userError } = await supabase
          ...
          .single();
        if (userError) {
          console.error('[LOAD] User role fetch error:', userError);
          throw new Error('Failed to fetch user role');
        }
        setUserRole(userData?.role);

        // If user is a couple, fetch their couple profile
        if (userData?.role === 'couple') {
          const { data: coupleData, error: coupleError } = await supabase
            ...
            .maybeSingle();
          if (coupleError) {
            console.error('[LOAD] Couple fetch error:', coupleError);
  ...
  const handleNextStep = () => {
    ...
    const requiredFields = step.fields.filter(f => 
      ['partner1Name', 'partner2Name', 'email', 'servicePackage', 'timeframe', 'message'].includes(f)
    );
...
${inquiryForm.message}
      ...
      if (fetchThreadError && fetchThreadError.code !== 'PGRST116') throw fetchThreadError;
    ...
    <div className="max-w-6xl mx-auto px-4 py-8">
      ...
      <div className="bg-white rounded-lg shadow-sm p-4 mb-8 flex flex-wrap gap-6 items-center justify-between">
        <div className="flex items-center gap-4">
          {vendor.website_url && (
            <a href={vendor.website_url} target="_blank" rel="noopener noreferrer" className="text-gray-600 hover:text-primary" title="Website">
              <Globe className="w-5 h-5" />
            </a>
          )}
          {vendor.facebook_url && (
            <a href={vendor.facebook_url} target="_blank" rel="noopener noreferrer" className="text-gray-600 hover:text-primary" title="Facebook">
              <Facebook className="w-5 h-5" />
            </a>
          )}
          {vendor.instagram_url && (
            <a href={vendor.instagram_url} target="_blank" rel="noopener noreferrer" className="text-gray-600 hover:text-primary" title="Instagram">
              <Instagram className="w-5 h-5" />
            </a>
          )}
          {vendor.youtube_url && (
            <a href={vendor.youtube_url} target="_blank" rel="noopener noreferrer" className="text-gray-600 hover:text-primary" title="YouTube">
          ...
          <div className="bg-white rounded-lg shadow-sm p-6 space-y-4">
            ...
            <div className="space-y-4">
              ...
              <div className="flex items-center justify-between">
                ...
                <span>{vendor.videos.length}</span>
              </div>
            </div>
