...
import { MessageSquare, Check, X, ArrowRight, Calendar, Store, MapPin, Star, Users2, BookOpen } from 'lucide-react';
...
const VendorLeads = () => {
  ...
  const [stats, setStats] = useState<LeadStats>({ totalLeads: 0, totalBooked: 0 });
  ...
  const loadLeads = async () => {
    try {
      ...
      setStats({
        ...
        totalBooked: leadsData?.filter(lead => lead.status === 'booked').length || 0
      });
    } catch (error) {
  ...
  const handleStartMessage = async (lead: Lead) => {
    try {
      ...
      const { error: messageError } = await supabase
        ...
        .insert({
          sender_id: user.id,
          receiver_id: lead.couple.user_id,
          content: "Thank you for your interest! I'd love to work with you. Let's discuss your wedding plans in more detail.",
        ...
        <div className="bg-white p-6 rounded-lg shadow-sm">
          <div className="flex items-center space-x-4">
            ...
            <div>
              ...
              <p className="text-2xl font-semibold">{stats.totalBooked}</p>
            </div>
          </div>
        </div>
      </div>
