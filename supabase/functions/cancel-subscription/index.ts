...
serve(async (req: Request) => {
  ...
  try {
    ...
    // Update the subscriptions table to reflect the pending cancellation
    ...
    return new Response(
      JSON.stringify({
        message: `Subscription scheduled to cancel at the end of the billing period on ${new Date(subscription.current_period_end * 1000).toISOString()}`,
        subscription,
      }),
      {
        headers: { ...corsHeaders, 'Content-Type': 'application/json' },
