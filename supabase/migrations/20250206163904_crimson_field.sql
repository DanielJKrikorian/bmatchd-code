...
BEGIN
  ...
  DROP POLICY IF EXISTS "users_signup_policy_v1" ON users;
  
  -- Drop policies for vendors table
  DROP POLICY IF EXISTS "enable_signup_access" ON vendors;
  DROP POLICY IF EXISTS "vendors_signup_policy_v1" ON vendors;
  
  -- Drop policies for couples table
  DROP POLICY IF EXISTS "enable_signup_access" ON couples;
  DROP POLICY IF EXISTS "couples_signup_policy_v1" ON couples;
END $$;

-- Create new policies with unique names
CREATE POLICY "users_access_policy_v3"
  ON users
  FOR ALL
  USING (true)
  WITH CHECK (true);

CREATE POLICY "vendors_access_policy_v3"
  ON vendors
  FOR ALL
  USING (true)
  WITH CHECK (true);

CREATE POLICY "couples_access_policy_v3"
  ON couples
  FOR ALL
  USING (true)
  WITH CHECK (true);
