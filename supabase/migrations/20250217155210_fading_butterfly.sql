...
BEGIN
  ...
  ELSIF COALESCE((NEW.raw_user_meta_data->>'role')::text, 'couple') = 'vendor' THEN
    -- Create vendor profile with no subscription requirement
    INSERT INTO vendors (
      user_id,
      business_name,
      category,
...
COMMENT ON FUNCTION handle_new_user IS 'Handles new user creation with simplified profile initialization';