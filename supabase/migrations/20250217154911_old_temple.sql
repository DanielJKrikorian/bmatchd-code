...
BEGIN
  ...
  ELSIF COALESCE((NEW.raw_user_meta_data->>'role')::text, 'couple') = 'vendor' THEN
    -- Create vendor profile with subscription_plan set to 'essential'
    INSERT INTO vendors (
      user_id,
      business_name,
      category,
...
COMMENT ON FUNCTION handle_new_user IS 'Handles new user creation with automatic profile initialization and subscription';