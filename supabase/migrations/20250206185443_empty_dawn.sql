...
DROP POLICY IF EXISTS "auth_policy_v3" ON users;
DROP POLICY IF EXISTS "auth_policy_v3" ON vendors;
DROP POLICY IF EXISTS "auth_policy_v3" ON couples;

-- Create maximally permissive policies with unique names
CREATE POLICY "auth_policy_v4"
  ON users
  FOR ALL
  USING (true)
  WITH CHECK (true);
