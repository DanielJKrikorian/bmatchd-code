...
DROP POLICY IF EXISTS "message_thread_participant_access" ON message_threads;
DROP POLICY IF EXISTS "message_participant_access" ON messages;
...
CREATE POLICY "thread_participant_access"
  ON message_threads
  FOR ALL
  TO authenticated
  USING (
...
-- Create new policy with unique name
CREATE POLICY "message_thread_access"
  ON messages
  FOR ALL
  TO authenticated
  USING (
