...
CREATE OR REPLACE FUNCTION send_notification_email_v1()
RETURNS TRIGGER AS $$
DECLARE
  v_email text;
...
CREATE TRIGGER on_notification_created
  ...
  FOR EACH ROW
  EXECUTE FUNCTION send_notification_email_v1();

-- Add comment to document the changes
COMMENT ON FUNCTION send_notification_email_v1 IS 'Sends email notifications through Edge Function when new notifications are created';