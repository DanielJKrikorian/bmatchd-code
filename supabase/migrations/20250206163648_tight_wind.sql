  ...
  LOOP
    ...
    EXCEPTION WHEN OTHERS THEN
      IF v_retries >= v_max_retries THEN
        ...
        RETURN NULL; -- Return NULL to prevent the trigger from completing
      END IF;
...
BEGIN
  ...
  DROP POLICY IF EXISTS "users_signup_access_policy" ON users;
  DROP POLICY IF EXISTS "users_unrestricted_access" ON users;
  ...
  DROP POLICY IF EXISTS "vendors_signup_access_policy" ON vendors;
  DROP POLICY IF EXISTS "vendors_unrestricted_access" ON vendors;
  
  -- Drop policies for couples table
  DROP POLICY IF EXISTS "enable_signup_access" ON couples;
  DROP POLICY IF EXISTS "allow_all_access" ON couples;
  DROP POLICY IF EXISTS "couples_signup_access_policy" ON couples;
  DROP POLICY IF EXISTS "couples_unrestricted_access" ON couples;
END $$;

-- Create new policies with unique names
CREATE POLICY "users_signup_policy_v1"
  ON users
  FOR ALL
  USING (true)
  WITH CHECK (true);

CREATE POLICY "vendors_signup_policy_v1"
  ON vendors
  FOR ALL
  USING (true)
  WITH CHECK (true);

CREATE POLICY "couples_signup_policy_v1"
  ON couples
  FOR ALL
  USING (true)
  WITH CHECK (true);
