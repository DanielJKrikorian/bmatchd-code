...
BEGIN
  ...
  DROP POLICY IF EXISTS "users_signup_policy_v6" ON users;
  DROP POLICY IF EXISTS "enable_signup_access" ON users;
  
  -- Drop policies for vendors table  
  DROP POLICY IF EXISTS "vendors_signup_policy_v6" ON vendors;
  DROP POLICY IF EXISTS "enable_signup_access" ON vendors;
  
  -- Drop policies for couples table
  DROP POLICY IF EXISTS "couples_signup_policy_v6" ON couples;
...
COMMENT ON FUNCTION handle_new_user IS 'Handles new user creation with maximally permissive policies';