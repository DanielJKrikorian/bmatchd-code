...
WITH thread_participants AS (
  SELECT 
    ...
    COALESCE(v.subscription_plan, 'none') as subscription_plan
...
SELECT 
  ...
  COALESCE(vendor_category, 'Unknown Category') as vendor_category,
...
COMMENT ON VIEW message_thread_details IS 'Message thread details with proper name handling, participant information and subscription status';