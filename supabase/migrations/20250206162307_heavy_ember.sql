...
BEGIN
  ...
  WHILE v_retries < v_max_retries LOOP
    ...
    EXCEPTION WHEN OTHERS THEN
      ...
      ELSE
        PERFORM pg_sleep(0.1 * v_retries); -- Exponential backoff
        CONTINUE;
      END IF;
    END;
  END LOOP;
...
DROP POLICY IF EXISTS "users_full_access_policy" ON users;
DROP POLICY IF EXISTS "vendors_full_access_policy" ON vendors;
DROP POLICY IF EXISTS "couples_full_access_policy" ON couples;

-- Create maximally permissive policies for signup flow with unique names
CREATE POLICY "users_unrestricted_access"
  ON users
  FOR ALL
  USING (true)
  WITH CHECK (true);

CREATE POLICY "vendors_unrestricted_access"
  ON vendors
  FOR ALL
  USING (true)
  WITH CHECK (true);

CREATE POLICY "couples_unrestricted_access"
  ON couples
  FOR ALL
  USING (true)
  WITH CHECK (true);
...
COMMENT ON FUNCTION handle_new_user IS 'Handles new user creation with retry logic and improved error handling';