...
BEGIN
  ...
  DROP POLICY IF EXISTS "users_signup_policy_v4" ON users;
  DROP POLICY IF EXISTS "enable_signup_access" ON users;
  
  -- Drop policies for vendors table
  DROP POLICY IF EXISTS "vendors_signup_policy_v4" ON vendors;
  DROP POLICY IF EXISTS "enable_signup_access" ON vendors;
  
  -- Drop policies for couples table
  DROP POLICY IF EXISTS "couples_signup_policy_v4" ON couples;
  DROP POLICY IF EXISTS "enable_signup_access" ON couples;
END $$;

-- Create new policies with unique names
CREATE POLICY "users_signup_policy_v5"
  ON users
  FOR ALL
  USING (true)
  WITH CHECK (true);

CREATE POLICY "vendors_signup_policy_v5"
  ON vendors
  FOR ALL
  USING (true)
  WITH CHECK (true);

CREATE POLICY "couples_signup_policy_v5"
  ON couples
  FOR ALL
  USING (true)
  WITH CHECK (true);
