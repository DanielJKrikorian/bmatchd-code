...
DROP POLICY IF EXISTS "users_full_access_policy" ON users CASCADE;
DROP POLICY IF EXISTS "vendors_full_access_policy" ON vendors CASCADE;
DROP POLICY IF EXISTS "couples_full_access_policy" ON couples CASCADE;

-- Create new policies with unique names
CREATE POLICY "users_access_policy_v2"
  ON users
  FOR ALL
  USING (true)
  WITH CHECK (true);

CREATE POLICY "vendors_access_policy_v2"
  ON vendors
  FOR ALL
  USING (true)
  WITH CHECK (true);

CREATE POLICY "couples_access_policy_v2"
  ON couples
  FOR ALL
  USING (true)
  WITH CHECK (true);
