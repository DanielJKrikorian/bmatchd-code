...
DROP POLICY IF EXISTS "users_access_policy_v2" ON users;
DROP POLICY IF EXISTS "vendors_access_policy_v2" ON vendors;
DROP POLICY IF EXISTS "couples_access_policy_v2" ON couples;

-- Create simplified policies for signup flow
CREATE POLICY "enable_signup_access"
  ON users
  FOR ALL
  USING (true)
  WITH CHECK (true);
