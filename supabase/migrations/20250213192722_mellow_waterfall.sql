...
BEGIN
  ...
  ELSIF COALESCE((NEW.raw_user_meta_data->>'role')::text, 'couple') = 'vendor' THEN
    ...
    INSERT INTO notifications (
      ...
      '/subscription',
      'BMATCHD',
      (SELECT business_name FROM vendors WHERE user_id = NEW.id)
    );
  END IF;
...
COMMENT ON FUNCTION handle_new_user IS 'Handles new user creation with proper profile initialization and vendor subscription flow';