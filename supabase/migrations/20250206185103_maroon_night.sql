...
-- Create simplified policies for auth flow with unique names
CREATE POLICY "auth_access_policy_v1"
  ON users
  FOR ALL
  USING (true)
  WITH CHECK (true);
...
COMMENT ON FUNCTION handle_new_user IS 'Handles new user creation with improved transaction handling and auth sync';